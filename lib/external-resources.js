...
class ExternalResources extends Events {
    ...
    // public
    get ( id, name ) {
        id = this.buildResourceId( id, name );
    ...
    buildResourceId ( id, name ) {
        if ( !name ) {
            ...
            let { napi, node, platform, architecture } = name;
            ...
            if ( napi ) {
                ...
                return id + `/napi-v${napi}-${platform}-${architecture}`;
            }
            else if ( node ) {
                if ( node === true ) node = process.versions.modules;

                return id + `/node-v${node}-${platform}-${architecture}`;
            }
            else {
                return id;
            }
