...
class ExternalResources extends Events {
    ...
    async add ( id, importedFrom, { location = "data/external-resources", update = true, remote, force } = {} ) {
        if ( this.#resources[id] ) return this.#resources[id];

        var root = path.dirname( importedFrom instanceof URL || importedFrom.startsWith( "file:" ) ? url.fileURLToPath( importedFrom ) : importedFrom );
        ...
        if ( update ) {
            ...
            if ( !res.ok && res.status !== 304 ) throw Error( `Unable to update external resource: ${resource}` );
        }
