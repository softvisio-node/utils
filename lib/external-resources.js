...
class ExternalResources extends Events {
    ...
    // private
    #parseResourceId ( id, { location, detectCaller } ) {
        ...
        // global
        if ( location === "global" ) {
            location = path.join( env.getXdgDataDir( GLOBAL_LOCATION ), `${ owner }-${ repo }-${ tag }`, name );
        }
    ...
    #getCaller () {
        ...
        Error.captureStackTrace( trace, this.#findPackageRoot );
