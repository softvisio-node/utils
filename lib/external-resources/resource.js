...
const DEFAULT_CHECK_TIMEOUT = 1000 * 60 * 60 * 24, // 24 hours
    ...
    GLOBAL_LOCATION = "softvisio/external-resources";
...
export default class ExternalResource extends Events {
    ...
    constructor ( id, packageRoot, { autoUpdate, location } = {} ) {
        super();
        ...
        // global
        if ( location === "global" ) {
            this.#location = path.join( env.getXdgDataDir( GLOBAL_LOCATION ), `${ this.#owner }-${ this.#repo }-${ this.#tag }`, this.#name );
        }
    ...
    async #readIndex () {
        ...
        if ( index ) {
            ...
            if ( index.lastUpdated ) index.lastUpdated = new Date( index.lastUpdated );
