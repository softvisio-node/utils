...
export default class ExternalResource extends Events {
    ...
    async update ( { remote = true, force, silent, cache } = {} ) {
        ...
        if ( !remoteIndex ) {
            res = await fetch( this.#remoteIndexUrl ).catch( e => result.catch( e, { "keepError": false } ) );
        ...
        // update
        res = await fetch( this.#downloadUrl ).catch( e => result.catch( e, { "keepError": false } ) );
        ...
        res = await new Promise( resolve => {
            ...
            writable.once( "error", e => resolve( result.catch( e, { "keepError": false } ) ) );
