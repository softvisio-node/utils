...
export default class ExternalResource extends Events {
    ...
    async update ( { remote = true, force, silent, cache } = {} ) {
        ...
        // resource exists
        if ( !remote && localIndex.tag ) return result( 200 );

        let remoteIndex = cache?.[this.#remoteIndexUrl];

        // download remote index
        if ( !remoteIndex ) {
            res = await fetch( this.#remoteIndexUrl ).catch( e => result.catch( e ) );
            ...
            if ( !remoteIndex ) return result( [500, `Remote index not found`] );
        ...
        remoteIndex[this.#name].lastChecked = new Date();

        // update is not required
        if ( !force && remoteIndex[this.#name].etag === localIndex.etag ) {
            this.#writeIndex( remoteIndex[this.#name] );

            return result( 304 );
        }

        // update
        res = await fetch( this.#downloadUrl ).catch( e => result.catch( e ) );
        ...
        remoteIndex[this.#name].lastUpdated = new Date();
