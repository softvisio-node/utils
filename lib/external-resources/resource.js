...
export default class ExternalResource extends Events {
    ...
    constructor ( id, root, { autoUpdate } = {} ) {
        super();
    ...
    async update ( { remote = true, force, silent, cache } = {} ) {
        ...
        // resource exists
        if ( !remote && localIndex.etag ) return result( 200 );

        let remoteIndex = cache?.[this.#remoteIndexUrl];

        // download remote index
        if ( !remoteIndex ) {
            res = await fetch( this.#remoteIndexUrl ).catch( e => result.catch( e, { "silent": true } ) );
        ...
        // update
        res = await fetch( this.#downloadUrl ).catch( e => result.catch( e, { "silent": true } ) );
        ...
        res = await new Promise( resolve => {
            ...
            writable.once( "error", e => resolve( result.catch( e, { "silent": true } ) ) );
