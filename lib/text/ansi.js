...
const ANSI_KEEP_STYLES_REGEXP = new RegExp( /(?:\x1B\[|\u009B)[\x30-\x3F]*[\x20-\x2F]*[\x40-\x6Cn~]/, "g" ); // ANSI SGR code ends with the "m"
const RESET = "\x1B[0m";
...
class ANSI {
    ...
    defineColor ( name, color ) {
        ...
        ANSI.defineAnsiCode( "on" + name.charAt( 0 ).toUpperCase() + name.slice( 1 ), createColorCode( 48, color ), RESET_BACKGROUND );
    }
...
for ( const name in codes ) {
    ANSI.defineAnsiCode( name, "\x1B[" + codes[ name ][ 0 ] + "m", codes[ name ][ 1 ]
        ? "\x1B[" + codes[ name ][ 1 ] + "m"
        : null );
}
